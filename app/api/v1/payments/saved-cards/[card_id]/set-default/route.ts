...
function getBackendUrl(cardId: string) {
  ...
  if (!base) return "";
  const suffix = `/payments/saved-cards/${encodeURIComponent(cardId)}/set-default`;
  if (base.endsWith("/api/v1")) return `${base}${suffix}`;
  return `${base}/api/v1${suffix}`;
}

export async function PATCH(request: NextRequest, ctx: { params: Promise<{ card_id: string }> }) {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Failed to set default saved card", details: text },
    ...
    console.error("❌ Set default saved card proxy (v1) error:", error);
